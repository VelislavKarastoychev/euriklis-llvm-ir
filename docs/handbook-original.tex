...
The classical compiler theory says \(M\) languages targeting N architectures demand \(M \times N\) back‑ends. In practice, LLVM collapses that to \(M\) front‑ends plus thin target descriptions, because the
  bulk of the back‑end (instruction selection, scheduling, register allocation, emission) is
  shared in LLVM itself; you parameterize the backend with data layout, calling convention,
  and a modest set of target hooks rather than re‑implementing a full code generator per
  architecture. This reduce the compilation process complexity from \(M \times N\) to  \(M + N\).
